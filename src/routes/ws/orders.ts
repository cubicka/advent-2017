...
function List(req: express.Request, res: express.Response, next: express.NextFunction) {
    const storecode = req.didi.usercode;
...
function Print(req: express.Request, res: express.Response, next: express.NextFunction) {
    const id = req.params.id;

    return MarkTransaction(id)
    .then(() => {
        res.send({ message: 'Transactions is marked as printed.' });
    });
}
