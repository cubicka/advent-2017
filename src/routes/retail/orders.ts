...
import { CreateTransaction, GetTransaction } from '../../model/transactions';
import { IsNumber, IsString, Middleware } from '../../util/validation';
...
function Create(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    const { items, picktime, remarks, storecode } = req.body;
    ...
    return CreateTransaction(storecode, usercode, details, items)
    .then(() => {
...
function List(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    return GetTransaction(usercode)
    .then(transactions => {
